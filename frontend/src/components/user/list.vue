  ...
  methods: {
    ...
    search: function () {
      ...
      API.findList(params).then(function (result) {
        ...
        that.loading = false
        that.$message.error({showClose: true, message: err.toString(), type: error, duration: 2000})
      }).catch(function (error) {
        that.loading = false
        console.log(error)
        that.$message.error({showClose: true, message: '请求出现异常', type: 'error', duration: 2000})
      })
    }
  },
  mounted() {
