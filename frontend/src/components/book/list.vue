<template>
  <div>
    ...
    <div class="card">
      <div class="title space"> 亲子阅读指导 </div>
  ...
  methods: {
    ...
    submit: function() {
      ...
      for (let i = 0; i < this.words.length; ++i) {
        if (
          !Tools.check_fetal(this.words[i].text, '重点单词文本', String(i + 1))
        ) {
          return
        }
        my_values.append('word_text', this.words[i].text)
        if (
          !Tools.check_fetal(this.words[i].audio, '重点单词音频', String(i + 1))
        ) {
          return
        }
      ...
      for (let i = 0; i < this.pages.length; ++i) {
        ...
        if (
          !Tools.check_fetal(this.pages[i].audio, '书籍内容配音', String(i + 1))
        ) {
          return
        }
